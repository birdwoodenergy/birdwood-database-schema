...
BEGIN TRY
    ...
    IF EXISTS (SELECT 1 FROM dbo.company WHERE name = 'ASSM Strata - Auto Xcel')
    BEGIN
        PRINT 'Company "ASSM Strata - Auto Xcel" already exists. Skipping insert.';
    END
    ELSE
    BEGIN
        INSERT INTO dbo.company (name, address, timezone, latitude, longitude, contact_number, email_address)
        VALUES ('ASSM Strata - Auto Xcel', 'unit 4/5 Rigg Place, Bonnyrigg NSW, Australia', 'Australia/NSW', NULL, NULL, NULL, NULL);
        PRINT 'Inserted company "ASSM Strata - Auto Xcel".';
    END;
    ...
    DECLARE @company_name NVARCHAR(50) = 'ASSM Strata - Auto Xcel';
    ...
    BEGIN
        SET @grid_device_endpoint = 'DD34108929920';
    ...
    BEGIN
        SET @pv_device_endpoint = 'DD34108929920';
