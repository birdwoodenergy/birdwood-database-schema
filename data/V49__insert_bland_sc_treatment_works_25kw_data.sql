...
BEGIN TRY
    ...
    IF EXISTS (SELECT 1 FROM dbo.company WHERE name = 'Bland SC - Treatment Works 25kW')
    BEGIN
        PRINT 'Company "Bland SC - Treatment Works 25kW" already exists. Skipping insert.';
    END
    ELSE
    BEGIN
        INSERT INTO dbo.company (name, address, timezone, latitude, longitude, contact_number, email_address)
        VALUES ('Bland SC - Treatment Works 25kW', 'Neeld Street, West Wyalong NSW, Australia', 'Australia/NSW', NULL, NULL, NULL, NULL);
        PRINT 'Inserted company "Bland SC - Treatment Works 25kW".';
    END;
    ...
    DECLARE @company_name NVARCHAR(50) = 'Bland SC - Treatment Works 25kW';
    ...
    BEGIN
        SET @grid_device_endpoint = 'DD34108195204';
    ...
    BEGIN
        SET @pv_device_endpoint = 'DD34108195204';
