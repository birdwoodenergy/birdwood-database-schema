...
BEGIN TRY
    ...
    IF EXISTS (SELECT 1 FROM dbo.company WHERE name = 'BCC- Kids Early Learning Minchinbury')
    BEGIN
        PRINT 'Company "BCC- Kids Early Learning Minchinbury" already exists. Skipping insert.';
    END
    ELSE
    BEGIN
        INSERT INTO dbo.company (name, address, timezone, latitude, longitude, contact_number, email_address)
        VALUES ('BCC- Kids Early Learning Minchinbury', '46 Minchin Drive, Minchinbury NSW, Australia', 'Australia/NSW', NULL, NULL, NULL, NULL);
        PRINT 'Inserted company "BCC- Kids Early Learning Minchinbury".';
    END;
    ...
    DECLARE @company_name NVARCHAR(50) = 'BCC- Kids Early Learning Minchinbury';
    ...
    BEGIN
        SET @grid_device_endpoint = 'DDD3710131027';
        ...
        SET @grid_device_endpoint = 'DDF5335307294';
        IF EXISTS (SELECT 1 FROM dbo.device WHERE company_data_provider_id = @grid_company_data_provider_id AND type = 'GRID' AND endpoint = @grid_device_endpoint)
        BEGIN
            PRINT 'Device (GRID) with endpoint "' + @grid_device_endpoint + '" already exists for "' + @company_name + '". Skipping insert.';
        END
    ...
    BEGIN
        SET @pv_device_endpoint = 'DDD3710131027';
        ...
        SET @pv_device_endpoint = 'DDF5335307294';
        IF EXISTS (SELECT 1 FROM dbo.device WHERE company_data_provider_id = @pv_company_data_provider_id AND type = 'PV' AND endpoint = @pv_device_endpoint)
        BEGIN
            PRINT 'Device (PV) with endpoint "' + @pv_device_endpoint + '" already exists for "' + @company_name + '". Skipping insert.';
        END
