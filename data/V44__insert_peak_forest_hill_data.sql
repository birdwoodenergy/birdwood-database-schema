...
BEGIN TRY
    ...
    IF EXISTS (SELECT 1 FROM dbo.company WHERE name = 'Peak Forest Hill')
    BEGIN
        PRINT 'Company "Peak Forest Hill" already exists. Skipping insert.';
    END
    ELSE
    BEGIN
        INSERT INTO dbo.company (name, address, timezone, latitude, longitude, contact_number, email_address)
        VALUES ('Peak Forest Hill', '58 Allonby Avenue, Forest Hill NSW, Australia', 'Australia/NSW', NULL, NULL, NULL, NULL);
        PRINT 'Inserted company "Peak Forest Hill".';
    END;
    ...
    DECLARE @company_name NVARCHAR(50) = 'Peak Forest Hill';
    ...
    BEGIN
        SET @grid_device_endpoint = 'DDF5335176853';
    ...
    BEGIN
        SET @pv_device_endpoint = 'DDF5335176853';
