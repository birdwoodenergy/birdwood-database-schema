...
BEGIN TRY
    ...
    IF EXISTS (SELECT 1 FROM dbo.company WHERE name = 'South Hay Hotel')
    BEGIN
        PRINT 'Company "South Hay Hotel" already exists. Skipping insert.';
    END
    ELSE
    BEGIN
        INSERT INTO dbo.company (name, address, timezone, latitude, longitude, contact_number, email_address)
        VALUES ('South Hay Hotel', '7 Nailor Street, Hay South NSW, Australia', 'Australia/NSW', NULL, NULL, NULL, NULL);
        PRINT 'Inserted company "South Hay Hotel".';
    END;
    ...
    DECLARE @company_name NVARCHAR(50) = 'South Hay Hotel';
    ...
    BEGIN
        SET @grid_device_endpoint = 'DDB5335304273';
    ...
    BEGIN
        SET @pv_device_endpoint = 'DDB5335304273';
