...
BEGIN TRY
    ...
    IF EXISTS (SELECT 1 FROM dbo.company WHERE name = 'Bland SC - Treatment Works 10kW')
    BEGIN
        PRINT 'Company "Bland SC - Treatment Works 10kW" already exists. Skipping insert.';
    END
    ELSE
    BEGIN
        INSERT INTO dbo.company (name, address, timezone, latitude, longitude, contact_number, email_address)
        VALUES ('Bland SC - Treatment Works 10kW', 'Neeld Street, West Wyalong NSW, Australia', 'Australia/NSW', NULL, NULL, NULL, NULL);
        PRINT 'Inserted company "Bland SC - Treatment Works 10kW".';
    END;
    ...
    DECLARE @company_name NVARCHAR(50) = 'Bland SC - Treatment Works 10kW';
    ...
    BEGIN
        SET @grid_device_endpoint = 'DDC4108183286';
        ...
        SET @grid_device_endpoint = 'DD63710142261';
        IF EXISTS (SELECT 1 FROM dbo.device WHERE company_data_provider_id = @grid_company_data_provider_id AND type = 'GRID' AND endpoint = @grid_device_endpoint)
        BEGIN
            PRINT 'Device (GRID) with endpoint "' + @grid_device_endpoint + '" already exists for "' + @company_name + '". Skipping insert.';
        END
    ...
    BEGIN
        SET @pv_device_endpoint = 'DDC4108183286';
        ...
        SET @pv_device_endpoint = 'DD63710142261';
        IF EXISTS (SELECT 1 FROM dbo.device WHERE company_data_provider_id = @pv_company_data_provider_id AND type = 'PV' AND endpoint = @pv_device_endpoint)
        BEGIN
            PRINT 'Device (PV) with endpoint "' + @pv_device_endpoint + '" already exists for "' + @company_name + '". Skipping insert.';
        END
