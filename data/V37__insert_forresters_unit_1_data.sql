...
BEGIN TRY
    ...
    IF EXISTS (SELECT 1 FROM dbo.company WHERE name = 'Forresters - Unit 1 - Drift Home & Living')
    BEGIN
        PRINT 'Company "Forresters - Unit 1 - Drift Home & Living" already exists. Skipping insert.';
    END
    ELSE
    BEGIN
        INSERT INTO dbo.company (name, address, timezone, latitude, longitude, contact_number, email_address)
        VALUES ('Forresters - Unit 1 - Drift Home & Living', '15-17 Forresters Beach Road, Forresters Beach NSW, Australia', 'Australia/NSW', NULL, NULL, NULL, NULL);
        PRINT 'Inserted company "Forresters - Unit 1 - Drift Home & Living".';
    END;
    ...
    DECLARE @company_name NVARCHAR(50) = 'Forresters - Unit 1 - Drift Home & Living';
    ...
    BEGIN
        SET @grid_device_endpoint = 'DD24108745541';
    ...
    BEGIN
        SET @pv_device_endpoint = 'DD24108745541';
