...
BEGIN TRY
    ...
    IF EXISTS (SELECT 1 FROM dbo.company WHERE name = 'Gundie Bluie')
    BEGIN
        PRINT 'Company "Gundie Bluie" already exists. Skipping insert.';
    END
    ELSE
    BEGIN
        INSERT INTO dbo.company (name, address, timezone, latitude, longitude, contact_number, email_address)
        VALUES ('Gundie Bluie', '516 Irrigation Way, Narrandera NSW, Australia', 'Australia/NSW', NULL, NULL, NULL, NULL);
        PRINT 'Inserted company "Gundie Bluie".';
    END;
    ...
    DECLARE @company_name NVARCHAR(50) = 'Gundie Bluie';
    ...
    BEGIN
        SET @grid_device_endpoint = 'DDC5335304259';
    ...
    BEGIN
        SET @pv_device_endpoint = 'DDC5335304259';
