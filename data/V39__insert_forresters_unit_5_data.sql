...
BEGIN TRY
    ...
    IF EXISTS (SELECT 1 FROM dbo.company WHERE name = 'Forresters - Unit 5 -Brown Sugar Bakery')
    BEGIN
        PRINT 'Company "Forresters - Unit 5 -Brown Sugar Bakery" already exists. Skipping insert.';
    END
    ELSE
    BEGIN
        INSERT INTO dbo.company (name, address, timezone, latitude, longitude, contact_number, email_address)
        VALUES ('Forresters - Unit 5 -Brown Sugar Bakery', '15-17 Forresters Beach Road, Forresters Beach NSW, Australia', 'Australia/NSW', NULL, NULL, NULL, NULL);
        PRINT 'Inserted company "Forresters - Unit 5 -Brown Sugar Bakery".';
    END;
    ...
    DECLARE @company_name NVARCHAR(50) = 'Forresters - Unit 5 -Brown Sugar Bakery';
    ...
    BEGIN
        SET @grid_device_endpoint = 'DD64108743152';
        ...
        SET @grid_device_endpoint = 'DD65335178185';
        IF EXISTS (SELECT 1 FROM dbo.device WHERE company_data_provider_id = @grid_company_data_provider_id AND type = 'GRID' AND endpoint = @grid_device_endpoint)
        BEGIN
            PRINT 'Device (GRID) with endpoint "' + @grid_device_endpoint + '" already exists for "' + @company_name + '". Skipping insert.';
        END
    ...
    BEGIN
        SET @pv_device_endpoint = 'DD64108743152';
        ...
        SET @pv_device_endpoint = 'DD65335178185';
        IF EXISTS (SELECT 1 FROM dbo.device WHERE company_data_provider_id = @pv_company_data_provider_id AND type = 'PV' AND endpoint = @pv_device_endpoint)
        BEGIN
            PRINT 'Device (PV) with endpoint "' + @pv_device_endpoint + '" already exists for "' + @company_name + '". Skipping insert.';
        END
