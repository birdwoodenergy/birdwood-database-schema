...
BEGIN TRY
    ...
    IF EXISTS (SELECT 1 FROM dbo.company WHERE name = 'Trinity Medical Centre')
    BEGIN
        PRINT 'Company "Trinity Medical Centre" already exists. Skipping insert.';
    END
    ELSE
    BEGIN
        INSERT INTO dbo.company (name, address, timezone, latitude, longitude, contact_number, email_address)
        VALUES ('Trinity Medical Centre', '54 Best Street, Wagga Wagga NSW, Australia', 'Australia/NSW', NULL, NULL, NULL, NULL);
        PRINT 'Inserted company "Trinity Medical Centre".';
    END;
    ...
    DECLARE @company_name NVARCHAR(50) = 'Trinity Medical Centre';
    ...
    BEGIN
        SET @grid_device_endpoint = 'DD55335297525';
    ...
    BEGIN
        SET @pv_device_endpoint = 'DD55335297525';
