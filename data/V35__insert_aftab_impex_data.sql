...
BEGIN TRY
    ...
    IF EXISTS (SELECT 1 FROM dbo.company WHERE name = 'Aftab Impex')
    BEGIN
        PRINT 'Company "Aftab Impex" already exists. Skipping insert.';
    END
    ELSE
    BEGIN
        INSERT INTO dbo.company (name, address, timezone, latitude, longitude, contact_number, email_address)
        VALUES ('Aftab Impex', 'unit 7/71A Milperra Road, Revesby NSW, Australia', 'Australia/NSW', NULL, NULL, NULL, NULL);
        PRINT 'Inserted company "Aftab Impex".';
    END;
    ...
    DECLARE @company_name NVARCHAR(50) = 'Aftab Impex';
    ...
    BEGIN
        SET @grid_device_endpoint = 'DDA4108743691';
    ...
    BEGIN
        SET @pv_device_endpoint = 'DDA4108743691';
