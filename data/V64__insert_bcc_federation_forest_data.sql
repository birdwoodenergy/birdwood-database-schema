...
BEGIN TRY
    ...
    IF EXISTS (SELECT 1 FROM dbo.company WHERE name = 'BCC- Federation Forest')
    BEGIN
        PRINT 'Company "BCC- Federation Forest" already exists. Skipping insert.';
    END
    ELSE
    BEGIN
        INSERT INTO dbo.company (name, address, timezone, latitude, longitude, contact_number, email_address)
        VALUES ('BCC- Federation Forest', 'Great Western Highway & Simpson Hill Road, Mount Druitt NSW, Australia', 'Australia/NSW', NULL, NULL, NULL, NULL);
        PRINT 'Inserted company "BCC- Federation Forest".';
    END;
    ...
    DECLARE @company_name NVARCHAR(50) = 'BCC- Federation Forest';
    ...
    BEGIN
        SET @grid_device_endpoint = 'DDA3710131360';
    ...
    BEGIN
        SET @pv_device_endpoint = 'DDA3710131360';
