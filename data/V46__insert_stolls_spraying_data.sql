...
BEGIN TRY
    ...
    IF EXISTS (SELECT 1 FROM dbo.company WHERE name = 'Stolls Spraying')
    BEGIN
        PRINT 'Company "Stolls Spraying" already exists. Skipping insert.';
    END
    ELSE
    BEGIN
        INSERT INTO dbo.company (name, address, timezone, latitude, longitude, contact_number, email_address)
        VALUES ('Stolls Spraying', 'unit 1/2-18 Cheshire Street, Wagga Wagga NSW, Australia', 'Australia/NSW', NULL, NULL, NULL, NULL);
        PRINT 'Inserted company "Stolls Spraying".';
    END;
    ...
    DECLARE @company_name NVARCHAR(50) = 'Stolls Spraying';
    ...
    BEGIN
        SET @grid_device_endpoint = 'DD55335313892';
    ...
    BEGIN
        SET @pv_device_endpoint = 'DD55335313892';
